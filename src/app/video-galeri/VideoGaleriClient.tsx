...
function VideoCard({ video, index }: { video: typeof videos[0]; index: number }) {
    ...
    useEffect(() => {
        ...
        const capture = () => { hv.currentTime = 1.5; };
        const draw = () => {
        ...
        >
            ...
            <div className={styles.playerWrapper}>
                {!playing ? (
                    ...
                    <video ref={videoRef} src={video.src} controls className={styles.videoPlayer} autoPlay />
                )}
            </div>
        </motion.div>
    );
...
export default function VideoGaleriClient() {
    return (
